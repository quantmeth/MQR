...
# Import the package "pathanalysis"
...
rm <- mediation(model = mpg ~ hp ~ wt, 
          data = mtcars, 
          standardized = TRUE)
icl <- rm$`CI Lower 95 %`[4]
icu <- rm$`CI Upper 95 %`[4]
eee <- rm$Estimate[4]
```


> Une analyse de médiation avec bootstrap est effectuée pour tester le lien indirect le lien indirect de `wt` à `mpg` par l'intermédiaire de `hp`. Les résultats montrent un lien indirect significatif de `r eee`, [95%IC\ `r icl`,\ `r icu`]. La Figure\ \@ref(fig:mtmed) présente les résultats.

```{r mtmed, fig.cap="Résultats de l'analyse de  médiation",echo=FALSE, fig.align="center",out.height="75%" , out.width="75%"}
knitr::include_graphics("image//mtcars_med.png")
```
