...
1. Le problème de Monty Hall tel qu'éminemment décrit.
...
<div class="figure">
<img src="12-Simuler_files/figure-html/histmh2-1.png" alt="Histogramme des estimations des échantillons" width="672" />
<p class="caption">(\#fig:histmh2)Histogramme des estimations des échantillons</p>
</div>
...
La fonction principale est `apply()`. Elle nécessite le jeu de données (`X`) sur lequel appliquer la fonction (`FUN`) et un argument (`MARGIN`) pour identifier la dimension selon laquelle il faut appliquer la fonction, comme `MARGIN = 1` produit l'opération par lignes; `MARGIN = 2` produit l'opération par colonnes.^[Il y a aussi des cas plus avancés lorsque le jeu possède plus de deux dimensions, comme `MARGIN = c(1, 2)` produit l'opération sur une matrice (lignes par colonnes) ou `MARGIN = n` produit l'opération sur la n^e^ dimension.]
