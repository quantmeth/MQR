...
Dans cet exemple, les variances de `x1` et de `x3` sont égales à 2 et 3 respectivement. Si ces deux variables sont additionnées pour créer `y`, alors, selon l'équation\ \@ref(eq:eq2), la variance de `y` est de $\sigma^2_{y}=\sigma^2_{x_{1}} + \sigma^2_{x_2} = `r s.x1^2` + `r s.x2^2`= `r s.x1^2+ s.x2^2`$
...
$$\sigma_{x_1,x_2} = \rho_{x_1,x_2} \sigma_{x_1} \sigma_{x_2} = `r rho` \times `r s.x1` \times `r s.x2` = `r s.x1*s.x2 *rho`,$$
...
$$\sigma_y^2=\sigma_{x_1}^2+\sigma_{x_2}^2+2\sigma_{x_1,x_2}= `r s.x1^2`+`r s.x2^2` + 2 \times `r s.x1x2`  = `r s.x1^2 + s.x2^2 + 2*s.x1x2`,$$
...
```{r tri1111, echo = FALSE, fig.cap = "Représentation des variables sous forme de triangles.", out.height="75%" , out.width="75%", fig.align = "center"}
knitr::include_graphics("image//tri.png")
```
...
L'équation\ \@ref(eq:eq8) donne $\sigma_y^2=\beta^2 \sigma_x^2=`r beta`^2 \times `r s.x1^2`= `r beta^2 * s.x1^2`$, ce qui est identique.
...
\begin{aligned}
\sigma_y^2 &= \beta_1^2 \sigma_{x_1}^2+\beta_2^2 \sigma_{x_2}^2+2\beta_1 \beta_2 \sigma_{x_1,x_2} \\ & =
`r beta1`^2 \times `r s.x1^2` + `r beta2`^2 \times `r s.x2^2` + 2 \times `r beta1` \times `r beta2` \times `r s.x1x2` \\ & = `r beta1^2 * s.x1^2 + beta2^2 * s.x2^2 + 2 * beta1 * beta2 * s.x1x2`
\end{aligned}
$$
...
```{r mod3, fig.cap="Modèle à trois variables.",echo=FALSE, fig.align="center", out.height="50%", out.width="50%"}
knitr::include_graphics("image//mod3.png")
```
