...
```{r regaf, echo = FALSE, fig.cap = "Les représentations de la régression et l'analyse factorielle.", out.height="75%" , out.width="75%", fig.align = "center"}
knitr::include_graphics("image//reg_af.png")
```
...
```{r plotacp, echo = FALSE, fig.cap = "Présentation des données (`jd.acp`).", out.height="75%" , out.width="75%", fig.align = "center"}
jd.acp <- as.data.frame(jd.acp)
res <- coef(lm(x2~x1, data = jd.acp))
plot(jd.acp, xlim = c(-2,2), ylim = c(-2,2), pch = 19, bty = "l", lwd = 2, yaxs="i", xlab = "Variable 1", ylab = "Variable 2")
...
```{r acpsuj, echo = FALSE, fig.align='center',fig.cap='Les représentations des participants selon les variables (gauche) ou selon les composantes principales (droite).', out.height="75%", out.width="75%"}
r.svd <- svd(jd.acp)
participants <- r.svd$u %*% diag(r.svd$d)
ld <- res.eig$vectors %*% diag(sqrt(res.eig$values))
#axes <- r#r.svd$v %*% diag(r.svd$d)
par(mfrow = c(1, 2), pty="s")
plot(jd.acp, xlim = c(-2.5,2.5), ylim = c(-2.5,2.5), pch = 19, lwd = 1, yaxs="i", col=(1:n), main = "Plan des variables", xlab = "Variable 1", ylab = "Variable 2",bty = "l")
segments(x0 = -2, y0 = 0, y1 = 0, x1 = 2, lty = "dashed")
segments(x0 = 0, y0 = -2, y1 = 2, x1 = 0, lty = "dashed")
text(x = .60, y = 2.2, labels = "axe2")
text(x = 2, y = -.20, labels = "axe1")
plot(participants, xlim = c(-2.5,2.5), ylim = c(-2.5,2.5), pch = 19, lwd = 1, yaxs="i", col=(1:n), main = "Plan des composantes", xlab = "Composante 1", ylab = "Composante 2",bty = "l")
segments(x0 = -2, y0 = ld[2,1] / ld[1,1] * -2, x1 = 2, y1 = ld[2,1] / ld[1,1] * 2, lty = "dashed")
segments(x0 = -2, y0 = ld[2,2] / ld[1,2] * -2, x1 = 2, y1 = ld[2,2] / ld[1,2] * 2, lty = "dashed")
text(x = 2.3, y= ld[2,2] / ld[1,2] * - 2-.5, labels = "axe2", srt = -47)
text(x = 1.75, y= -ld[2,2] / ld[1,2] * -2, labels = "axe1", srt = -47)
```
...
```{r varacp, echo = FALSE, fig.align='center', fig.cap="La représentation des variables selon les composantes principales.", out.height="75%", out.width="75%"}
plot(ld, xlim = c(-1,1), ylim = c(-1,1), pch = 19, bty = "l", lwd = 2, yaxs="i", xlab = "Composante 1", ylab = "Composante 2", pty = "s")
segments(x0 = -2, y0 = 0, y1 = 0, x1 = 2, col = "grey")
segments(x0 = 0, y0 = -2, y1 = 2, x1 = 0, col = "grey")
arrows(x0 = c(0,0), 
       y0 = c(0,0), 
       x1 = ld[,1]-.02,
       y1 = ld[,2]-c(-.02,.02), length = .1)
text(x = ld[1,1]-.1, y = ld[1,2]-.15, labels = "Variable 1")
text(x = ld[2,1]-.1, y = ld[2,2]+.15, labels = "Variable 2")
```
...
```{r polycarf, echo = FALSE, out.height="75%", out.width="75%",fig.align="center", fig.cap="L'équation du polynôme caractéristique."}
x = seq(0, 2, by = .01)
y = x^2 - 2 * x + .36
plot(x, y, pty = "s", bty = "l", type = "l", lwd = 2, xlab = expression(lambda), ylab = expression(paste(italic("f"),"(",lambda,")")))
segments(x0 = -1, y0 = 0, x1=3, y1 = 0, lwd = 2)
points(x = .2, y = 0, pch = 19)
points(x = 1.8, y = 0, pch = 19)
text(x = .3, y = .1, labels=expression(paste(lambda[2]," = .2")))
text(x = 1.7, y = .1, labels=expression(paste(lambda[1]," = 1.8")))
```
...
#generate random correlation matrix based on the eigenvalues
#https://github.com/cran/fungible/blob/master/R/rMAP.R
rMAP <- function(l, maxiter = 5000, eps = 1e-12){
  delta <- 1
  iter <- 0
  p <- length(l)
  ...
  while(delta > eps){
    ...
    diag(Su) <- 1
    QLQ <- eigen(Su)
    delta <- sqrt(sum((l - QLQ$values)^2))
    iter <- iter + 1
    if(iter >= maxiter) break
    S <- QLQ$vectors %*% diag(l) %*% t(QLQ$vectors)
    S <- .5 * (S + t(S))
  }
  S
}
l <- c(1:3) / sum(1:3) * 3
eigen(rMAP(l=l))$values
l
```
